...
import com.ydh.signight.repository.UserRepository
...
class SecurityService(
    ...
    // JWT_SECRET
    private val JWT_SECRET: SecretKey = Keys.hmacShaKeyFor(jwt_secret.toByteArray())
    // 토큰 유효시간
    ...
    fun checkValidToken(header: ServerRequest.Headers): Mono<Boolean> {
        return try {
            ...
            val claims = Jwts.parser().verifyWith(JWT_SECRET).build().parseSignedClaims(token)
            Mono.just(true)
        } catch (e: Exception) {
   ...
   fun checkValidHeader(mono: ServerRequest.Headers): Mono<String?> {
    ...
    fun getLoggedInUser(req: ServerRequest): Mono<User> {
        return try {
            ...
            val username = Jwts.parser().verifyWith(JWT_SECRET).build().parseSignedClaims(token).payload.subject
